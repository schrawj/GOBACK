...
ma <- read_dta('//smb-main.ad.bcm.edu/genepi2/GOBACK/Datasets/raw_data/massachusetts_birth_defect_data_20210924.dta') %>% 
...
ma.bd <- ma %>% select(studyid, starts_with('DXCode')) 
...
mi <- readRDS('//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Old_datasets/Michigan/2021 update/michigan.icd9.birth.defect.codes.v20230403.rds')

#' ICD9 codes were used to record birth defect diagnoses until 10/2015.
#' For now, I will append older data with ICD9 codes.
mi <- mi %>% 
  filter(birth.yr > 1991) %>% 
  select(studyid, birth.yr, birth.mo, bdefect.icd9cod1:bdefect.icd9cod30) %>% 
  filter(birth.yr < 2014 | (birth.yr == 2015 & birth.mo < 10)) %>% 
...
mi %<>% rename_with(~paste0('icd9code',1:30), .cols = starts_with('bdefect')) %>% 
  mutate(state = 'MI',
         coding.system = 'ICD-9') %>% 
  select(studyid, state, birth.yr, birth.mo, coding.system, icd9code1:icd9code30)

mi <- mi %>% 
  mutate(dob = as.Date(paste0(birth.yr, '-', birth.mo, '-', '15'))) %>% 
  select(studyid, state, dob, coding.system, icd9code1:icd9code30)

saveRDS(mi, 
        '//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_MI_icd9_20230410.rds')

#' Load ICD9 existing ICD9 codes and append new MI data.
icd9 <- readRDS("//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_icd9_states.v20230216.rds") %>% 
  filter(state != 'MI') %>% 
  bind_rows(mi)

saveRDS(icd9, 
        "//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_icd9_states_20230410.rds")

rm(list = ls()); gc()

# Append updated MI data (ICD10) ------------------------------------------

#' New Michigan data.
mi <- readRDS('//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Old_Datasets/Michigan/mi.raw.data.v20220511.rds')

#' ICD10 codes were used to record birth defect diagnoses beginning 10/2015.
mi %<>% 
  select(STUDYID, DOB, BDefect_ICD10COD1:BDefect_ICD10COD30) %>% 
  filter(DOB >= '2015-10-01') %>% 
  mutate(STUDYID = paste0('mi', STUDYID))

names(mi) %<>% tolower() %<>% str_replace_all('_', '.')

#' Kids w/o ICD10 codes are represented by "". Set all to "".
#mi %<>% mutate(across(bdefect.icd9cod1:bdefect.icd9cod30, ~ str_replace(.x, '\\Q.\\E', '')))

#' There are some seven digit codes where it appears two birth defects were entered in the same cell or some such. Keep first 4 digits, then drop any trailing letters (e.g., in Q02Q673).
mi %<>% mutate(across(bdefect.icd10cod1:bdefect.icd10cod30, ~ ifelse(str_length(.x) == 7, substr(.x, 1, 4), .x)))
mi %<>% mutate(across(bdefect.icd10cod1:bdefect.icd10cod30, ~ str_remove(.x, '[[:alpha:]]$')))

#' For codes longer than 3 digits, add a period after the third.
mi %<>% mutate(across(bdefect.icd10cod1:bdefect.icd10cod30, ~ ifelse(str_length(.x) > 3, paste0(substr(.x, 1, 3), '.', substr(.x, 4, nchar(.x)) ), .x ) ))

#' Drop kids without any birth defects (i.e., kids for whom every ICD10 code == "").
mi %<>% 
  mutate(flag = as.logical(if_all(bdefect.icd10cod1:bdefect.icd10cod30, ~ifelse(.x == '', 1, 0)))) %>% 
  filter(flag == F) %>% 
  select(-flag)

#' Change " to NA to match existing data.
mi %<>% mutate(across(bdefect.icd10cod1:bdefect.icd10cod30, ~ ifelse(.x == "", NA, .x)))

#' Drop kids without at least one defect in the ranges that contribute to our computed BD variables.
mi %<>% 
  mutate(flag = ifelse(if_any(bdefect.icd10cod1:bdefect.icd10cod30, 
                              ~str_detect(.x, '^Q')), 1, 0)) %>% 
  filter(flag == 1)

#' Sort BD columns such that each child's Q codes come first, with NAs at the end.
#' This is slow, since I'm updating every cell of a large matrix individually.

#' Names for the thirty BD columns.
my.cols <- paste0('bdefect.icd10cod',1:30)

tic()
for (i in 1:nrow(mi)){
  
  # Generate a sorted vector of the ith child's BD codes, with Q codes at the beginning, other codes in the middle, and NAs at the end.
  index.codes <- mi %>% select(bdefect.icd10cod1:bdefect.icd10cod30) %>% slice(i) %>% c() %>% unlist()
  
  q.codes <- subset(index.codes, str_detect(index.codes, '^Q')) %>% sort()
  
  other.codes <- subset(index.codes, str_detect(index.codes, '^Q', negate = T)) %>% sort()
  
  index.codes <- c(q.codes, other.codes)
  index.codes <- c(index.codes, rep(NA, times = 30 - length(index.codes)))
...
mi %<>% 
  rename_with(~paste0('icd10code',1:30), .cols = starts_with('bdefect')) %>% 
  mutate(state = 'MI',
         coding.system = 'ICD-10') %>% 
  select(studyid, state, coding.system, dob, icd10code1:icd10code30)

#' As of 4/11/2023, MI is the only state to be included in this data frame.
saveRDS(mi, 
        '//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_icd10_states_20230410.rds')
...
sc <- read_dta('//smb-main.ad.bcm.edu/genepi3/TiffanyChambers/GOBACK Data/South Carolina Data/SC GOBACK Data_harmonized birth cert (all variables).dta') %>% 
  mutate(dob = as.Date(paste0(.$BIRTH_YR,'-07-02'), format = '%Y-%m-%d'),
         coding.system = 'ICD-9/ICD-10')

names(sc) %<>% tolower() %>% str_replace_all('_', '.')

not.blank <- function(x){ x != '' }

#' Select BD columns and filter to affected children.
sc.codes <- sc %>% 
  filter(if_any(starts_with('bd.dx'), not.blank)) %>% 
  select(studyid, state, coding.system, dob, starts_with('bd.dx')) %>% 
  mutate(studyid = paste0('sc',studyid))

#' Trim white spaces and reformat ICD10 codes.
sc.codes %<>%
  mutate(across(starts_with('bd.dx'), ~ str_trim(.x, side = 'both'))) %>% 
  mutate(across(starts_with('bd.dx'), ~ ifelse(grepl('^Q', .x) & str_length(.x) > 3, 
                                               paste0(substr(.x,1,3),'.', substr(.x,4,nchar(.x))), 
                                               .x)))

#' Reformat ICD9 codes.
sc.codes %<>%
  mutate(across(starts_with('bd.dx'), ~ ifelse(str_detect(.x, '^[:digit:]') & str_length(.x) > 3, paste0(substr(.x,1,3), '.', substr(.x, 4, nchar(.x))), .x)))

#' Change blank cells from "" to NA.
sc.codes %<>% mutate(across(starts_with('bd.dx'), ~ ifelse(.x == "", NA, .x)))

#' Drop kids without at least one defect in the ranges that contribute to our computed BD variables.
sc.codes %<>% 
  mutate(icd9.flag = ifelse(if_any(starts_with('bd.dx'), 
                              ~str_detect(.x, str_c('^216', '^237.7', '^7[45]', sep = '|'))), 1, 0),
         
         icd10.flag = ifelse(if_any(starts_with('bd.dx'),
                              ~str_detect(.x, '^Q')), 1, 0)) %>% 
  filter(icd9.flag == 1 | icd10.flag == 1) %>% 
  select(-ends_with('flag')) %>% 
  rename_with(~paste0('icdcode',1:11), starts_with('bd.dx'))

saveRDS(sc.codes, 
        '//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_mixed_coding_states.v20220810.rds')
...
#' Expand number of BD columns in SC data to fit NJ.
bd.codes <- readRDS('//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_mixed_coding_states.v20220810.rds')

for (i in 12:31){
  
  new.col <- paste0('icdcode',i)
  
  bd.codes <- bd.codes %>% mutate('{new.col}' := as.character(NA))
  
}

nj <- readRDS('//smb-main.ad.bcm.edu/genepi3/JeremySchraw/GOBACK/Datasets/nj.birth.defect.raw.data.rds')

#' Add metadata about coding systems, state, DOB; standardize BD code column formatting; select relevant columns and rename them to match other states.
nj.clean <- nj %>% 
  mutate(state = 'NJ',
         coding.system = 'ICD-9/ICD-10',
         across(starts_with('icd'), ~ ifelse(.x == "", NA, .x)),
         across(starts_with('icd9'), ~ ifelse(nchar(.x) == 3, paste0(.x,'.0'), .x)),
         across(starts_with('icd10'), ~ ifelse(str_detect(.x, '\\.$'), paste0(.x, '0'), .x))) %>% 
  select(studyid, state, coding.system, dob, starts_with('icd'))

names(nj.clean) <- c(names(nj.clean)[1:4], paste0('icdcode',1:43))

#' Arrange NJ BD code columns in alphanumeric order with NAs last/in the highest numbered columns.
#' Takes ~45 minutes.
for (i in 1:nrow(nj.clean)){
  
  sorted.cols <- nj.clean[i, 5:47] %>% as.vector() %>% unlist() %>% sort(na.last = T) %>% unique() 
  
  sorted.cols <- c(sorted.cols, rep(NA, times = 43 - length(sorted.cols))) %>% as.list()
  
  names(sorted.cols) <- paste0('idcode',1:43)
  
  nj.clean[i, 5:47] <- sorted.cols
  
}

#' Remove empty columns (code 32 and above).
nj.clean <- nj.clean %>% select(studyid:icdcode31)

bd.codes <- bind_rows(bd.codes, nj.clean)

saveRDS(bd.codes, '//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_mixed_coding_states_20230523.rds')  

# Discount hemangiomas ----------------------------------------------------

bds <- readRDS('//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_bpa_states.v20220603.rds')
...
#' Remove them to harmonize data structure with newer TX and MI data.
bds %<>% 
  mutate(flag = ifelse(if_any(bpa1:bpa63, ~str_detect(.x, str_c('^216', '^237.7', '^7[45]', sep = '|'))), 1, 0)) %>% 
  filter(!is.na(flag)) %>% 
  select(-flag)
...
codes <- readRDS('//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_icd9_states.v20220609.rds')

#' FL birth defects data were already cleaned and harmonized. See 'data_cleaning_FL_clean_and_prep_FL_data.R'
fl <- readRDS('//smb-main.ad.bcm.edu/genepi3/JeremySchraw/GOBACK/Datasets/fl.birth.defect.raw.data.rds')

codes <- bind_rows(codes, fl)

saveRDS(codes, '//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_icd9_states.v20230216.rds')

rm(list = ls()); gc()

# Corrections to OK spina bifida codes ------------------------------------

#' OK registry data.
load("//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Old_Datasets/Oklahoma/oklahoma.raw.data.rdata")

#' Cleaned BD codes for BPA states.
bpa <- readRDS('//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_bpa_states.v20220604.rds') %>% 
  filter(state != 'OK')
...
#' Standardize the names and values in each BD column.
ok <- ok %>% 
  mutate(across(starts_with('CODE'), ~ ifelse(.x == '.', NA, .x)),
         across(starts_with('CODE'), ~ str_remove_all(.x, '/'))) %>% 
  as_tibble()

#' Correct coding errors in a couple of cases that Priya and Lindsay Denson at OBDR identified.
ok <- ok %>% 
  mutate(CODE30 = case_when(studyid == 'ok392913' ~ '742100',
                            studyid == 'ok521330' ~ '741980',
                            .default = CODE30))

#' Add trailing zeroes when missing.
for (i in 2:ncol(ok)){
...
ok <- ok %>% select(all_of(names(bpa)))

bpa <- bind_rows(bpa, ok)

saveRDS(bpa,
        '//smb-main.ad.bcm.edu/genepi2/Old_genepi2/Jeremy/GOBACK/Datasets/Expanded_datasets/birth_defects_codes_bpa_states_20230417.rds')
